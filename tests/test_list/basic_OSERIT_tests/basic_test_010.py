...
try:
    ...
    test_name = "switch forcing file with variable current speed backward"
    test_descritpion = "Test if the model can switch files when they have variable current speed backward"
    ...
    delta_dist_lon = abs(result.variables['lon'][12] - result.variables['lon'][0] - dist_lon) + abs(result.variables['lon'][-1] - result.variables['lon'][-13])
    delta_dist_lat = abs(result.variables['lat'][-1] - result.variables['lat'][-13] - dist_lat) + abs(result.variables['lat'][12] - result.variables['lat'][0])
