...
try:
    ...
    test_name = "constant current at different depth"
    test_descritpion = "Compare drift at varying depth with a current varying with the depth"

    speed = 1 #m/s

    forcing_val = forcings()
    forcing_val.uo = [speed, speed, speed, -speed]
    forcing_val.vo = [speed, speed, speed, -speed]
    forcings_list = [forcing_val]
    req = get_default_request(id,path_ose)
    req["release"]["nopart"] = 1
    req["drift"]["iopt_3D"] = 1
    req["drift"]["iopt_cur_drift"] = 1
    cloud = get_simple_cloud()
    ...
    error = ""
    if  delta_dist_lat > abs(dist_lat) / 100: #1%error
        error += "underwater particle have not the good latitude\n"

    if  delta_dist_lon > abs(dist_lon) / 100: #1%error
        error += "underwater particle have not the good longitude\n"
