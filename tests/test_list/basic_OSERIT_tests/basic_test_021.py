...
try:
    ...
    test_name = "resurfacing"
    test_descritpion = "Test to iopt_resurfacing"
    ...
    if result.variables['part_state'][-1] != 1:
        error += f"particle state should stays in the water column when iopt_resurfacing = 0"
    
    #activating resurfacing
    ...
    if result.variables['part_state'][-1] != 0:
        error += f"particle state should be at the surface when iopt_resurfacing = 1"
