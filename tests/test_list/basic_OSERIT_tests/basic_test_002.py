...
try:
    ...
    test_name = "constant current"
    test_descritpion = "Test if a 1m current to the NORTHWEST push a particle to the correct location"
    ...
    error = ""
    if  delta_dist_lat > abs(dist_lat) / 1000: #0.1%error
        error += "particle have not the good latitude\n"

    if  delta_dist_lon > abs(dist_lon) / 1000: #0.1%error
